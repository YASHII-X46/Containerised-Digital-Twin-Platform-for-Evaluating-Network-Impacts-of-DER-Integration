...
class LoopbackTransport:
    """In-process synchronous transport used in tests."""
...
def _filter_to_subject(topic_filter: str) -> str:
    tokens = []
    for token in topic_filter.split("/"):
        tokens.append(">" if token == "#" else "*" if token == "+" else token)
    return ".".join(tokens)


class NatsTransport:
    """OpenFMB transport over NATS, bridged to the synchronous bus interface."""
    ...
    def publish(self, topic: str, payload: dict) -> None:
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._nc.publish(_topic_to_subject(topic), data),
            self._loop,
        )
        future.result(timeout=5)
