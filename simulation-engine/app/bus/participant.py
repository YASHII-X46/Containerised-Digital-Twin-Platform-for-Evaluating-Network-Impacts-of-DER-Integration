...
Turns a container into a first-class bus participant: it subscribes to its
command topics and answers with event messages, so the whole DT can be driven
over the OpenFMB bus instead of point-to-point HTTP.

Topic convention (under the configured prefix):
    {prefix}/command/{service}/{action}    e.g. openfmb/command/load-engine/generate
    {prefix}/event/{service}/{action}      e.g. openfmb/event/load-engine/generate

Every message carries an envelope with a ``correlationId`` so a requester can
match the event to its command.
"""
...
class BusParticipant:
    ...
    def on_command(self, action: str, handler: CommandHandler) -> None:
        """Register a handler: ``handler(payload) -> result_payload``."""
    ...
    def publish_event(self, action: str, payload: dict,
                      correlation_id: str | None = None, status: str = "ok") -> None:
        self._t.publish(
    ...
    # ---- client side (used by orchestrators / tests) ----------------------

    def request(self, target_service: str, action: str, payload: dict,
                timeout: float = 60.0) -> dict | None:
        """Publish a command to ``target_service`` and await its event."""
        corr = str(uuid.uuid4())
        result: dict = {}
